...
class ClienteViewSet(mixins.CreateModelMixin,
    ...
    def create(self, request, *args, **kwargs):
        ...
        if request.data['tipo'] == 'PF':
            conta = Conta.objects.create(cliente=Cliente.objects.get(cpf=request.data['cpf']))
        elif request.data['tipo'] == 'PJ':
            conta = Conta.objects.create(cliente=Cliente.objects.get(cnpj=request.data['cnpj']))
