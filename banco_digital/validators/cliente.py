    ...
    if data.get('tipo').upper() == 'PF':
            if (data.get('cnpj') is not None):
                raise ValidationError({'cnpj':
                    'Esse campo não deve ser passado para pessoa física'})
           
            if (data.get('cpf') is None):
                raise ValidationError({'cpf':
                    'Esse campo é obrigatório para pessoa física'})
       
    elif data.get('tipo').upper() == 'PJ':
        if (data.get('cpf') is not None):
            raise ValidationError({'cpf':
                'Esse campo não deve ser passado para pessoa jurídica'})
        
        if (data.get('cnpj') is None):
            raise ValidationError({'cnpj':
                'Esse campo é obrigatório para pessoa jurídica'})
